...
- `tarball`: *Optional*
   ...
   This is a boolean value and defaults to `false`. See [the behavior](#behavior) for more details.
...
- `versioned`: *Optional*
   ...
   This is a boolean value and defaults to `false`. If set to `true`, the
resource will read and write using the versioned KV Engine 2 and will display
the versioning tracked by vault instead of computing the sha256 hash of the
data.
...
the version will be the `sha256sum` hash of the json payload `{"foo":"bar}`.
  ...
  Uses vault's KV Engine 2 which supports versioning itself.
